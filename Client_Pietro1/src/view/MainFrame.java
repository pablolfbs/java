	...
	private void buildButtonPanel() {
		...
		deleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ClientTableModel model = (ClientTableModel) table.getModel();
				Client client = model.getRow(selectedRow);
				boolean done = ClientDAO.Remove(client); // método para remover
															// cliente do banco
															// de dados.
				//Verifica se a remoção foi concluída no Banco de dados.
				if (done) {

					removeClient(table.getSelectedRow());
				}else{
					JOptionPane.showMessageDialog(MainFrame.this, "Erro - 009 Não foi possivel excluir o Cliente pois não obteve conexão com o banco de dados.");
				}
			}
		});
